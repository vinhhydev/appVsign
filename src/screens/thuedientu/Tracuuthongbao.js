...
const Tracuuthongbao = () => {
  ...
  // Khởi tạo screen và set giá trị mặc định khi user thay đổi TG
  useEffect(() => {
    getStoredData();
  }, []);
  ...
  const storeData = async data => {
    try {
      ...
      await AsyncStorage.setItem('tdt_tb_tg', tg.toString());
  ...
  const getStoredData = async () => {
    const tg = await AsyncStorage.getItem('tdt_tb_tg');
    if (tg) {
  ...
  const tracuu = async () => {
    await axios
      .post(ehdUrl + wsStrings.TRACUUTBKT, {
        token: '@acc2k',
        mst: pickMst,
        pwd: decode(pass_tdt),
        ql: 'true',
      ...
      .then(response => {
        if (response.data.d) {
          ...
          if (res && res.length != 0) {
            const isError = res[0].sothongbao.includes('Error');
            if (isError) {
              Alert.alert('Lỗi', res[0].sothongbao);
            } else if (isError) {
              setData(res);
          ...
          <View style={styles.ehd.itemRow}>
            ...
            <TouchableOpacity
              ...
              <Icon
                name="eye-outline"
                size={35}
                color={tg ? '#0A68FE' : '#f9a726'}></Icon>
            </TouchableOpacity>
          </View>
          <Text style={styles.textH}>{item.sothongbao}</Text>
          <View style={styles.ehd.itemRow}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Header
        ...
        title={'Tra cứu thông báo tờ khai'}
      />
