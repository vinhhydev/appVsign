...
const Tracuugiaynoptien = () => {
  ...
  const storeData = async data => {
    try {
      ...
      await AsyncStorage.setItem('tdt_tcgnt_tg', tg.toString());
  ...
  const getStoredData = async () => {
    const tg = await AsyncStorage.getItem('tdt_tcgnt_tg');
    if (tg) {
  ...
  const tracuu = async () => {
    await axios
      .post(ehdUrl + wsStrings.TRACUUGNT, {
        token: '@acc2k',
        mst: pickMst,
        pwd: decode(pass_tdt),
        ql: 'true',
      ...
      .then(response => {
        ...
        if (res) {
          if (res[0].magiaodich.includes('Error')) {
            Alert.alert('Thông báo', res[0].magiaodich);
          } else {
            setData(res);
      ...
      <View>
        <View style={styles.ehd.itemWrapper}>
          <View style={styles.ehd.itemRow}>
            <Text style={styles.textH}>{item.ngaylapgnt}</Text>
            <TouchableOpacity
          ...
          <Text style={styles.textH}>{item.nganhang}</Text>
          <Text style={styles.text}>STK: {item.tknganhang}</Text>
          <Text style={styles.text}>
            Số tiền: {item.sotien} {item.loaitien}
          </Text>
          <Text style={styles.text}>Giờ nộp tiền: {item.ngaynopthue}</Text>
