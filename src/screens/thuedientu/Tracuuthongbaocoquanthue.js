...
const Tracuuthongbaocoquanthue = () => {
  ...
  // set giá trị mặc định khi user thay đổi TG
  useEffect(() => {
    if (data) {
      if (tg) {
        setQuy('1');
  ...
  const storeData = async data => {
    try {
      ...
      await AsyncStorage.setItem('tdt_tbcqt_tg', tg.toString());
  ...
  const getStoredData = async () => {
    const tg = await AsyncStorage.getItem('tdt_tbcqt_tg');
    if (tg) {
  ...
  const tracuu = async () => {
    await axios
      .post(ehdUrl + wsStrings.TRACUUTBCQT, {
        token: '@acc2k',
        mst: pickMst,
        pwd: decode(pass_tdt),
        ql: 'true',
  ...
  const getUrl = async item => {
    const url = 'https://ehoadondientu.com/upload/fid/' + item.fid;
    openPdf(url);
      ...
      <View>
        <View style={styles.ehd.itemWrapper}>
          <View style={styles.ehd.itemRow}>
            <Text style={styles.textH}>{item.ngaygui}</Text>
            <TouchableOpacity
          ...
          <Text style={styles.textH}>{item.tenthongbao}</Text>
          <View style={styles.ehd.itemRow}>
            <Text style={styles.text}>Cơ quan gửi: {item.coquangui}</Text>
          </View>
          <View style={styles.ehd.itemRow}>
            <Text></Text>
            <Text
...
export default Tracuuthongbaocoquanthue;
